...
export const StyleControls: React.FC<StyleControlsProps> = ({ config, onChange, readOnly = false }) => {
  ...
  const currentStructureSum =
    (config.pageStructure?.cover || 0) +
    (config.pageStructure?.directory || 0) +
    (config.pageStructure?.transition || 0) +
    (config.pageStructure?.content || 0) +
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 h-full">
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {STYLE_PRESETS.map(style => (
                  <button
                    ...
                    className={`text-[10px] px-2 py-1 rounded-full border transition-colors ${config.styleName === style
                ...
                <button
                  ...
                  className={`text-[10px] px-2 py-1 rounded-full border transition-colors ${!STYLE_PRESETS.includes(config.styleName)
                    ? 'bg-slate-800 text-white border-slate-800'
                    : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'
                    }`}
                >
              ...
              {readOnly ? (
                ...
                RATIO_PRESETS.map(ratio => (
                  <button
                    ...
                    className={`flex-1 text-xs px-2 py-1.5 rounded border text-center transition-colors ${config.aspectRatio === ratio
        ...
        <div className="col-span-1 flex flex-col h-full">
          ...
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 flex-1 flex flex-col">
            ...
            {/* Variant Count per Page */}
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm text-slate-600 font-medium">每页生成数</span>
              <div className="flex gap-2">
                {[1, 2, 4].map(count => (
                  <button
                    key={count}
                    onClick={() => onChange('defaultVariantCount', count)}
                    disabled={readOnly}
                    className={`px-3 py-1 text-sm font-medium rounded transition-all ${(config.defaultVariantCount || 1) === count
              ...
              <div className="flex items-center justify-between text-xs p-1.5 bg-white rounded border border-slate-100">
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    value={config.pageStructure?.transition || 0}
              ...
              <div className="flex items-center justify-between text-xs p-1.5 bg-white rounded border border-slate-100">
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    value={config.pageStructure?.content || 0}

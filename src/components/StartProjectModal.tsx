...
import { X, Play, FileText, Image as ImageIcon, CheckCircle2, AlertCircle, Clock } from 'lucide-react';
...
export const StartProjectModal: React.FC<StartProjectModalProps> = ({
  ...
  const completedItems = allItems.filter(i => i.status === 'success');
  const waitingItems = allItems.filter(i => i.status !== 'success');
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                共 <span className="text-blue-600 font-bold">{allItems.length}</span> 个页面，
                待生成 <span className="text-amber-600 font-bold">{waitingItems.length}</span> 个
                {completedItems.length > 0 && <span>，已完成 <span className="text-green-600 font-bold">{completedItems.length}</span> 个</span>}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-2">
            {allItems.map((item, index) => {
              ...
              return (
                <div key={item.id} className={`flex items-center gap-4 p-3 rounded-xl border transition-all group ${isCompleted
                  ? 'border-green-100 bg-green-50/30 hover:border-green-200'
                  : 'border-slate-100 hover:border-blue-200 hover:bg-slate-50'
                  }`}>
                  <span className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold font-mono ${isCompleted
                  ...
                  <div className={`text-xs font-bold flex items-center gap-1 ${isCompleted
                    ? 'text-green-500'
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-4">
          ...
          >
            进入项目 {waitingItems.length > 0 ? '(逐个生成)' : ''}
          </button>
          {waitingItems.length > 0 && (
            <button

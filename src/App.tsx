...
import { consumeAction, getActionCost, getBalance } from "./api/points";
...
const StyleTemplateManager = lazy(() => import('./components/StyleTemplateManager').then((module) => ({ default: module.StyleTemplateManager })));
const ProfileCenter = lazy(() => import('./components/user/ProfileCenter').then((module) => ({ default: module.ProfileCenter })));
const PointsHistory = lazy(() => import('./components/user/PointsHistory').then((module) => ({ default: module.PointsHistory })));
const AdminLayout = lazy(() => import('./components/admin').then((module) => ({ default: module.AdminLayout })));
const MessagesPage = lazy(() => import('./components/message').then((module) => ({ default: module.MessagesPage })));
...
const LazySectionFallback: React.FC<{ label?: string; fullHeight?: boolean }> = ({ label = '正在加载', fullHeight = false }) => (
  <div className={`flex items-center justify-center ${fullHeight ? 'h-full min-h-[320px]' : 'min-h-[220px]'} bg-slate-50`}>
...
// CascadingFilter 和 HistoryProjectCard 已移至 HistoryPage.tsx
...
const App: React.FC = () => {
  ...
  // --- Notification Polling (Only in Workbench) ---
  // 通知轮询主要用于快照摘要完成通知，仅在 workbench 模式下启用
  useEffect(() => {
    // 只在 workbench 模式下轮询，减少不必要的网络请求
  ...
  const syncProjectDataIntoWorkspace = useCallback((project: ProjectSession) => {
    isLoadingDataRef.current = true;
    setConfig(project.globalConfig);
    setItems(project.items);
    configRef.current = project.globalConfig;
    itemsRef.current = project.items;
    if (project.globalStyleMap) {
      setStyleMap(project.globalStyleMap);
      styleMapRef.current = project.globalStyleMap;
    }
    setHasUserInteraction(false);
    initialItemsLengthRef.current = project.items.length;
  ...
  useEffect(() => {
    // Load project data when switching projects in workbench OR agent mode
    // Agent mode needs this to show preview for completed projects
    if ((viewMode === 'workbench' || viewMode === 'agent') && currentProjectId && currentProjectId !== prevProjectIdRef.current) {
      if (currentProject) {
        syncProjectDataIntoWorkspace(currentProject);
  ...
  const [agentStyleSelectionCleared, setAgentStyleSelectionCleared] = useState(0);
  ...
  const [historyTargetProject, setHistoryTargetProject] = useState<ProjectSession | null>(null); // 用于 Dashboard 直接打开历史
  ...
  // 从 Dashboard 直接打开项目历史版本
  const handleOpenProjectHistory = (projectId: string) => {
    const project = projects.find(p => p.id === projectId);
    if (project) {
      setHistoryTargetProject(project);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans overflow-y-scroll">
      ...
      <AgentGlobalConfigModal
        ...
        onResetAll={() => {
          // 清除 Agent 模式的风格选中状态
          setAgentStyleSelectionCleared(prev => prev + 1);
      ...
      <HistorySidebar
        ...
        currentProject={historyTargetProject || currentProject || null}
        liveProjectData={historyTargetProject ? {
          ...historyTargetProject,
          items: historyTargetProject.items,  // 使用项目存储的数据
          globalConfig: historyTargetProject.globalConfig,
          globalStyleMap: historyTargetProject.globalStyleMap
        } : currentProject ? {
          ...currentProject,
      ...
      <ErrorBoundary>
        <div className={`bg-[#f8fafc] text-slate-800 font-sans selection:bg-rose-100 selection:text-rose-600 flex flex-col overflow-x-hidden ${viewMode === 'agent' ? 'h-screen' : 'min-h-screen'}`}>
                ...
                >
                  ...
                  <div className={`flex items-center ${isScrolled ? 'gap-3' : 'gap-6'} z-10 ${(viewMode === 'workbench' || viewMode === 'history-detail') ? 'flex-1 min-w-0' : 'shrink-0 w-auto xl:w-[300px]'}`}>
                    ...
                    {(viewMode === 'dashboard' || viewMode === 'history' || viewMode === 'templates' || viewMode === 'agent' || viewMode === 'trash') && (
                      <div className="flex items-center bg-slate-100/80 p-0.5 rounded-xl border border-slate-200/60 shrink-0">
                        <button
          ...
          {viewMode === "admin" ? (
            <Suspense fallback={<LazySectionFallback label="正在加载管理后台" fullHeight />}>
              <AdminLayout
            ...
            <Suspense fallback={<LazySectionFallback label="正在加载消息中心" fullHeight />}>
              <MessagesPage />
            </Suspense>
          ) : viewMode === "landing" ? (
            <LandingPage
            ...
            viewMode !== 'templates' && viewMode !== 'agent' ? (
              <main className="w-full max-w-[1480px] mx-auto px-6 py-6 space-y-8 flex-1">
                {viewMode === "dashboard" && (
                  <Suspense fallback={<LazySectionFallback label="正在加载创作室" />}>
                    <Dashboard
                      ...
                      onOpenHistory={handleOpenProjectHistory}
                      // Lifted States
                ...
                {viewMode === "trash" && (
                  <Suspense fallback={<LazySectionFallback label="正在加载回收站" />}>
                    <TrashPage
                            ...
                            {!isRequirementsPreview && (<>
                              ...
                              {config.previousRequirements && !isRefiningRequirements && !previewSnapshot && (
                                <button
                                  ...
                                  className="absolute bottom-3 right-[6.5rem] p-1.5 rounded-lg flex items-center gap-1.5 text-[10px] font-medium transition-all shadow-sm bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 hover:shadow-md"
                                  title="撤回修饰"
                                >
                                  <Undo2 size={12} />
                                  撤回修饰
                ...
                {viewMode === "history" && (
                  <Suspense fallback={<LazySectionFallback label="正在加载历史库" />}>
                    <HistoryPage
                      historySearchTerm={historySearchTerm}
                      setHistorySearchTerm={setHistorySearchTerm}
                      historyFilterStyle={historyFilterStyle}
                      setHistoryFilterStyle={setHistoryFilterStyle}
                      historyFilterRatio={historyFilterRatio}
                      setHistoryFilterRatio={setHistoryFilterRatio}
                      historyFilterPalette={historyFilterPalette}
                      setHistoryFilterPalette={setHistoryFilterPalette}
                      historyFilterPageType={historyFilterPageType}
                      setHistoryFilterPageType={setHistoryFilterPageType}
                      historyFilterMinPages={historyFilterMinPages}
                      setHistoryFilterMinPages={setHistoryFilterMinPages}
                      historyFilterMaxPages={historyFilterMaxPages}
                      setHistoryFilterMaxPages={setHistoryFilterMaxPages}
                      historyFilterTimeType={historyFilterTimeType}
                      setHistoryFilterTimeType={setHistoryFilterTimeType}
                      historyFilterStartDate={historyFilterStartDate}
                      setHistoryFilterStartDate={setHistoryFilterStartDate}
                      historyFilterEndDate={historyFilterEndDate}
                      setHistoryFilterEndDate={setHistoryFilterEndDate}
                      historyFilterTime={historyFilterTime}
                      setHistoryFilterTime={setHistoryFilterTime}
              ...
              <main className="flex-1 min-h-0 overflow-hidden">
                ...
                {viewMode === 'agent' && (
                  <Suspense fallback={<LazySectionFallback label="正在加载 AI 助手" fullHeight />}>
                    <div className="h-full bg-slate-50 flex flex-col">
                      ...
                      styleSelectionCleared={agentStyleSelectionCleared}
                      wsStatus={wsStatus}
                      wsMessage={wsMessage}
                      onWsJoinProject={joinProject}
                      onWsLeaveProject={leaveProject}
                      />
                    </div>
                  </Suspense>
                )}
                ...
                {viewMode === 'templates' && (
                  <Suspense fallback={<LazySectionFallback label="正在加载模板管理" fullHeight />}>

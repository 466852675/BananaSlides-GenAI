...
export const SharedStyleCard: React.FC<SharedStyleCardProps> = ({ 
  ...
  const thumbnailSrc = useMemo(() => {
    ...
    if ('sampleImages' in item && item.sampleImages && item.sampleImages.length > 0) {
      return item.sampleImages[0];
    }
    // Priority 2: styleMap.cover (File object, create ObjectURL)
    // CRITICAL FIX: explicit check for Blob/File to avoid crash on restored JSON data
    if (item.styleMap?.cover && item.styleMap.cover instanceof Blob) {
      return URL.createObjectURL(item.styleMap.cover);
    }
    // Priority 3: Legacy styleFile (from Preset)
    if ('styleFile' in item && item.styleFile && item.styleFile instanceof Blob) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all group flex flex-col h-full relative">
      ...
      <div className="p-4 pb-20 flex-1 flex flex-col space-y-3 relative">
        {/* Title & Metadata */}
        <div>
            <h4 className="font-bold text-slate-800 text-sm truncate mb-1" title={item.name}>
                {item.name || item.config.styleName}
            </h4>
            <div className="flex items-center text-[10px] text-slate-400 gap-2">
                <span className="bg-slate-100 px-1.5 py-0.5 rounded">{item.config.aspectRatio}</span>
                <span className="bg-slate-100 px-1.5 py-0.5 rounded">{item.config.colorPalette}</span>
            </div>
        </div>

        {/* Structure Tags (The "Favorite" feature) */}
        <div className="flex flex-wrap gap-1.5">
            {item.config.pageStructure.cover > 0 && <StructureTag label="封面" count={item.config.pageStructure.cover} color="purple" />}
            {item.config.pageStructure.directory > 0 && <StructureTag label="目录" count={item.config.pageStructure.directory} color="orange" />}
            {item.config.pageStructure.transition > 0 && <StructureTag label="过渡" count={item.config.pageStructure.transition} color="teal" />}
            {item.config.pageStructure.content > 0 && <StructureTag label="正文" count={item.config.pageStructure.content} color="indigo" />}
            {item.config.pageStructure.end > 0 && <StructureTag label="结束" count={item.config.pageStructure.end} color="slate" />}
        </div>
...
const StructureTag: React.FC<{ label: string; count: number; color: 'purple' | 'orange' | 'teal' | 'indigo' | 'slate' }> = ({ label, count, color }) => {
    const colorStyles = {
    ...
    return (
        <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${colorStyles[color]}`}>
            {label}{count}
        </span>
    );
};

        ...
        for (const order of user.orders) {
            if (order.status === 'PAID') {
                messages.push({
                    ...
                    bizType: 'order',
                    bizId: order.id,
                    actionUrl: `/user/orders?id=${order.id}`,
                    isImportant: true,
        ...
        for (const refund of user.refundRequests) {
            if (refund.status === 'COMPLETED') {
                messages.push({
                    ...
                    bizType: 'refund',
                    bizId: refund.id,
                    actionUrl: `/user/refunds?id=${refund.id}`,
                    isImportant: true,
        ...
        for (const project of user.projects) {
            ...
            if (Math.random() > 0.3) {
                messages.push({
                    ...
                    bizType: 'project',
                    bizId: project.id,
                    actionUrl: `/app?projectId=${project.id}`,
                    isImportant: false,
    ...
    for (const admin of admins) {
        ...
        for (const refund of pendingRefunds) {
            messages.push({
                ...
                bizId: refund.id,
                actionUrl: `/admin/orders?tab=refunds&refundId=${refund.id}`,
                isImportant: true,
                isRead: false,
